...
type ProdOrderLineComponent struct {
	...
	ExtendedDesc              string    `bun:"extended_desc,type:varchar(255),nullzero"`              // Extended description for this production order component.
	...
	QtyCanceled               float64   `bun:"qty_canceled,type:decimal(19,9),nullzero"`              // Component quantity canceled in SKUs.
	QtyUsed                   float64   `bun:"qty_used,type:decimal(19,9),nullzero"`                  // Component quantity processed in SKUs.
	...
	Disposition               string    `bun:"disposition,type:char(1),nullzero"`                     // Disposition of the unallocated quantity.
	...
	PoCost                    float64   `bun:"po_cost,type:decimal(19,9),nullzero"`                   // PO cost of the component. Used for direct ship, special and non-stock components.
	...
	CompExtendedDesc          string    `bun:"comp_extended_desc,type:varchar(255),nullzero"`         // Extended description of the component.
	...
	RequiredDate              time.Time `bun:"required_date,type:datetime,nullzero"`                  // The date in which this production order line component is required.
	ExpediteDate              time.Time `bun:"expedite_date,type:datetime,nullzero"`                  // The date in which this production order line component needs to be expedited
	...
	InventoryCost             float64   `bun:"inventory_cost,type:decimal(19,9),nullzero"`            // An inventory cost of a component based on inventory costing basis
	NewCost                   float64   `bun:"new_cost,type:decimal(19,9),nullzero"`                  // A user specified inventory cost of a component.  The default value is from inventory_cost
	...
	OriginalCompletionDate    time.Time `bun:"original_completion_date,type:datetime,nullzero"`       // Original expected completion date
	CompletionDate            time.Time `bun:"completion_date,type:datetime,nullzero"`                // Current expected completion date
	BackflushFlag             string    `bun:"backflush_flag,type:char(1),default:('N')"`             // Indicate if the component item is a backflush component.
	QtyScrapped               float64   `bun:"qty_scrapped,type:decimal(19,9),nullzero"`              // This column holds the sku qty that has been scrapped.
	CountryOfOrigin           string    `bun:"country_of_origin,type:varchar(255),nullzero"`          // Custom column to store item's country of origin in Production Order Processing Window
	BeltingLength             float64   `bun:"belting_length,type:decimal(19,9),nullzero"`            // Length of the cut for belting components
	BeltingWidth              float64   `bun:"belting_width,type:decimal(19,9),nullzero"`             // Width of the cut for belting components
	UomConvertedFlag          string    `bun:"uom_converted_flag,type:char(1),nullzero"`              // Custom - Indicates whether the uom is converted for this component
	LooseShipFlag             string    `bun:"loose_ship_flag,type:char(1),nullzero"`                 // Flag to indicate whether an assembly component on the production order is a loose ship item.
	ExtendedItemFlag          string    `bun:"extended_item_flag,type:char(1),nullzero"`
	UnitCostEdited            string    `bun:"unit_cost_edited,type:char(1),default:('N')"` // indicated if unit_cost get manually edited
	LaborTypeCd               int32     `bun:"labor_type_cd,type:int,nullzero"`             // Labor Type - Direct / Indirect
}
