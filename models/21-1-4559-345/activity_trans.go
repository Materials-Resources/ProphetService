...
type ActivityTrans struct {
	...
	AssignedToId          string    `bun:"assigned_to_id,type:varchar(30),nullzero"`                  // user id assigned to complete the activity
	CompletedDate         time.Time `bun:"completed_date,type:datetime,nullzero"`                     // Date the activity was completed -  or if the complet
	CompletedFlag         string    `bun:"completed_flag,type:char(1)"`                               // Check box showing whether activity has been comple
	CompletedById         string    `bun:"completed_by_id,type:varchar(30),nullzero"`                 // User id who completed the activity
	...
	ReminderTimeOffset    int32     `bun:"reminder_time_offset,type:int,nullzero"`                    // Time before target complete date reminder should display
	ReminderTimeOffsetCd  int32     `bun:"reminder_time_offset_cd,type:int,nullzero"`                 // Qualifies type of time the offset refers to
	...
	TransactionNo         string    `bun:"transaction_no,type:varchar(255),nullzero"`                 // Transaction reference
	Subject               string    `bun:"subject,type:varchar(255),nullzero"`                        // Overview of task
	CompanyId             string    `bun:"company_id,type:varchar(8),nullzero"`                       // Unique code that identifies a company.
	LinkId                float64   `bun:"link_id,type:decimal(19,0),nullzero"`                       // Unique code that elaborates the role the contact_id is playing for this task
	LinkTypeCd            int32     `bun:"link_type_cd,type:int,nullzero"`                            // Unique code which describes the link_id
	...
	DateLastSynched       time.Time `bun:"date_last_synched,type:datetime,nullzero"`                  // Indicates the date/time this record was last synched with outlook
	SerialNumberUid       int32     `bun:"serial_number_uid,type:int,nullzero"`                       // Indicates the serial number specific for this activity - FK to serial_number.
	AlarmCodeUid          int32     `bun:"alarm_code_uid,type:int,nullzero"`                          // Alarm code for task.
	ProblemCodeUid        int32     `bun:"problem_code_uid,type:int,nullzero"`                        // Problem code for task.
	OutlookTaskUid        string    `bun:"outlook_task_uid,type:varchar(255),nullzero"`               // Unique identifier for an outlook task.
	...
	StartDate             time.Time `bun:"start_date,type:datetime,nullzero"`                         // Start Date for the task
}
