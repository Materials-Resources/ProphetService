...
type Branch struct {
	...
	PayableGroupId            string    `bun:"payable_group_id,type:varchar(255),nullzero"`               // This column is no longer being used and is scheduled for removal in later version.
	ReceivableGroupId         string    `bun:"receivable_group_id,type:varchar(255),nullzero"`            // Identifier for receivable group.
	...
	RemitToAddressId          float64   `bun:"remit_to_address_id,type:decimal(19,0),nullzero"`           // Remit To Address information at the Branch level for printing on invoices
	LaborBillbackAccountNo    string    `bun:"labor_billback_account_no,type:varchar(32),nullzero"`       // Default Labor Bill Back Account Number for locations.
	...
	EmailSubjectPrefix        string    `bun:"email_subject_prefix,type:varchar(255),nullzero"`           // (Custom F80588) Prefix that will be added to the normal subject of form based email based on the branch of the transaction
}
