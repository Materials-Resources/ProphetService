...
type ProdOrderLine struct {
	...
	Labor                        float64   `bun:"labor,type:decimal(19,2),nullzero"`         // The labor amount for the line.
	...
	OverallAssemblyLength        float64   `bun:"overall_assembly_length,type:decimal(19,9),nullzero"`           // Used for hose assemblies. Not currently used.
	...
	CumAssemblyCost              float64   `bun:"cum_assembly_cost,type:decimal(19,9),nullzero"`                 // Stores the accumulated assembly cost that has been processed.
	...
	ExtendedDesc                 string    `bun:"extended_desc,type:varchar(255),nullzero"`                      // Extended description of the assembly item.
	...
	HoseUom                      string    `bun:"hose_uom,type:varchar(8),nullzero"`                  // FK to column unit_of_measure.unit_id - unit of measure chosen to describe the measurement of the overall length for this production order line.
	CreatedBy                    string    `bun:"created_by,type:varchar(255),default:(suser_sname())"`
	ExtdCompCost                 float64   `bun:"extd_comp_cost,type:decimal(19,4),nullzero"`              // A cost representing the sum of the components costs
	ExtdCompCostEditedFlag       string    `bun:"extd_comp_cost_edited_flag,type:char(1),nullzero"`        // This will be a Y/N field indicating whether or not the ext_comp_cost has been edited by the user
	DepositBin                   string    `bun:"deposit_bin,type:varchar(10),nullzero"`                   // bin where all the components will put to when assemble
	...
	RequiredDate                 time.Time `bun:"required_date,type:datetime,nullzero"`                    // The date in which this production order line item required.
	ExpediteDate                 time.Time `bun:"expedite_date,type:datetime,nullzero"`                    // The date in which this assembly line item needs to be expedited
	...
	CommittedStartDate           time.Time `bun:"committed_start_date,type:datetime,nullzero"`             // Date user commits to starting the assembly
	StartDate                    time.Time `bun:"start_date,type:datetime,nullzero"`                       // Date production order is printed
	EndDate                      time.Time `bun:"end_date,type:datetime,nullzero"`                         // Actual date assembly is finished
	CommentDate                  time.Time `bun:"comment_date,type:datetime,nullzero"`                     // User-defined date
	ProdOrderLineUid             int32     `bun:"prod_order_line_uid,type:int,unique"`                     // Unique Identifier for prod_order_line table
	DesignCompleteFlag           string    `bun:"design_complete_flag,type:char(1),nullzero"`              // Indicates whether the design of the Assembly has been completed. CAD Drawings, Blue Prints, etc…
	EstimatedProcessCost         float64   `bun:"estimated_process_cost,type:decimal(19,9),nullzero"`      // Estimated Secondary Processing Cost for the Assemblies.
	CountryOfOrigin              string    `bun:"country_of_origin,type:varchar(255),nullzero"`            // Custom column to store assembly components country of origin rolled up from all its components in Production Order Processing Window.
}
