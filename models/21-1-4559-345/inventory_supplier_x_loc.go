...
type InventorySupplierXLoc struct {
	...
	LocContractNumber          string    `bun:"loc_contract_number,type:varchar(40),nullzero"`             // contract number assigned at item/supplier level
	...
	DefaultDisposition         string    `bun:"default_disposition,type:char(1),nullzero"`                 // Default Disposition for consignment
	VmiStatus                  int32     `bun:"vmi_status,type:int,nullzero"`                              // Define a Primary Supplier/Location/Item as a Vendor Managed Inventory item
	CreatedBy                  string    `bun:"created_by,type:varchar(255),default:(suser_sname())"`
	OverrideBegDate            time.Time `bun:"override_beg_date,type:datetime,nullzero"`                  // The beginning date for the vmi status override
	OverrideEndDate            time.Time `bun:"override_end_date,type:datetime,nullzero"`                  // The ending date for the vmi status override
	VmiLastSendDate            time.Time `bun:"vmi_last_send_date,type:datetime,nullzero"`                 // The last date that vmi data was exported for the supplier/location
	OverrideVmiStatus          int32     `bun:"override_vmi_status,type:int,nullzero"`                     // Indicates the vmi status (manually done by customer) that overrides the system vmi status.
	...
	ManualLeadTime             int32     `bun:"manual_lead_time,type:int,nullzero"`                        // Override the Average Lead Time if set.
	FutureListPrice            float64   `bun:"future_list_price,type:decimal(19,9),nullzero"`             // Custom Column for Future List Price of an item
}
