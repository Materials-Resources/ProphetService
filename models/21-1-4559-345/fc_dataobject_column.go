...
type FcDataobjectColumn struct {
	...
	DataType              string    `bun:"data_type,type:varchar(255),nullzero"`                         // Data type for this column.
	TypeCd                int32     `bun:"type_cd,type:int"`                                             // Type of field, db column, db computed column, computed field.
	Expression            string    `bun:"expression,type:varchar(8000),nullzero"`                       // The expression of a db computed column or a computed field.
	RowStatusFlag         int32     `bun:"row_status_flag,type:int"`                                     // Status flag indicating whether the record is active or not.
	Description           string    `bun:"description,type:varchar(8000),nullzero"`                      // User friendly description for this column that can be used as a default label.
