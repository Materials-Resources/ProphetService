...
type BinType struct {
	...
	FrontCounterFlag string    `bun:"front_counter_flag,type:char(1),nullzero"`                     // Indicate a bin as Front Counter bin type
	BackflushFlag    string    `bun:"backflush_flag,type:char(1),default:('N')"`                    // Indicate if the bin is a backflush type or not.
	FullSkidOnlyFlag string    `bun:"full_skid_only_flag,type:char(1),nullzero"`                    // Indicates this type of bin contains only full skids
	ShippableFlag    string    `bun:"shippable_flag,type:char(1),nullzero"`                         // Indicates if user can take inventory from the bin to ship the orders.
}
