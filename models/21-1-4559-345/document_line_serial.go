...
type DocumentLineSerial struct {
	...
	SerialNumberUid       int32     `bun:"serial_number_uid,type:int,nullzero"`             // Uniquer row ID for the serial table.  Used as a staging area for unapproved transactions.
	...
	RfBinUid              int32     `bun:"rf_bin_uid,type:int,nullzero"`                    // Indicates the rf bin that picked the serial number.
	DepositBinUid         int32     `bun:"deposit_bin_uid,type:int,nullzero"`               // Keep the Slab Bin ID until the PO receipt is approved
}
