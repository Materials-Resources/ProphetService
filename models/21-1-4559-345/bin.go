...
type Bin struct {
	...
	DateLastCounted      time.Time `bun:"date_last_counted,type:datetime,nullzero"`                  // When was the inventory at this location last counted?
	CreatedBy            string    `bun:"created_by,type:varchar(255),default:(suser_sname())"`      // user who created bin record
	BinTypeUid           int32     `bun:"bin_type_uid,type:int,nullzero"`                            // Foreign key to the bin_type table
	...
	PutawayZoneUid       int32     `bun:"putaway_zone_uid,type:int,nullzero"`                        // foreing key to bin_zone_uid for putaway zone
	PutawayZoneSequence  int32     `bun:"putaway_zone_sequence,type:int,default:(0)"`                // Sequence bin has in putaway zone
	PickZoneUid          int32     `bun:"pick_zone_uid,type:int,nullzero"`                           // Foreign key - bin_zone_uid for pick zone
	...
	RfTerminalUid        int32     `bun:"rf_terminal_uid,type:int,nullzero"`                         // Pointer to the rf_terminal record that the bin record was created for
	...
	StageBinXDoorBinUid  int32     `bun:"stage_bin_x_door_bin_uid,type:int,nullzero"`                // Bin that will indicate this is a truck door bin for the stage bin.
	...
	MasterBinUid         int32     `bun:"master_bin_uid,type:int,nullzero"`                          // Indicates the master bin for this bin.
	...
	DateLastValidated    time.Time `bun:"date_last_validated,type:datetime,nullzero"`                // Last time BIN was validated
	LastValidatedBy      string    `bun:"last_validated_by,type:varchar(30),nullzero"`               // who validated it last time?
	AllowedAgeRangeWeeks int32     `bun:"allowed_age_range_weeks,type:int,nullzero"`                 // Custom F67642 - Indicates the range of manufacture dates (in weeks) allowed for a given item to be deposited into this bin.
}
