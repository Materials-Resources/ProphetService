...
type VendorRfqLine struct {
	...
	DeliveryInfo            string    `bun:"delivery_info,type:varchar(255),nullzero"`                     // Stores RFQ delivery information per line
	Responded               int32     `bun:"responded,type:int,default:(1104)"`                            // State of Vendor RFQ response
	RfqanalysisCompleteFlag string    `bun:"rfqanalysis_complete_flag,type:char(1),nullzero"`              // Determines if this RFQ should be excluded from future RFQ to Sales Quote analysis.
}
