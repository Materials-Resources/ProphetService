...
type InvAdjLine struct {
	...
	QtyCountedFlag         string    `bun:"qty_counted_flag,type:char(1),nullzero"`                     // Used by custom to indicate that the qty on an adjustment line created during physical count was counted.
	VendorId               float64   `bun:"vendor_id,type:decimal(19,0),nullzero"`                      // This is a custom column to specify vendor id for item qty to adjust in RMA receipt window
	...
	ParentTransLineNo      int32     `bun:"parent_trans_line_no,type:int,nullzero"`                     // Line Number of Associated Transaction
	...
	ReasonId               string    `bun:"reason_id,type:varchar(5),nullzero"`                // Custom (F65493): FK to reason.id.  Link to associated reason record.  For physical counts, the reason for this lines adjustment.
	Comment                string    `bun:"comment,type:varchar(255),nullzero"`                // Custom (F65493): user defined comment for this instance
