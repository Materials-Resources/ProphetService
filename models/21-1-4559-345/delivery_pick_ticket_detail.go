...
type DeliveryPickTicketDetail struct {
	...
	Disposition                 string    `bun:"disposition,type:char(1),nullzero"`           // Stores the disposition of the item in the case of an undershipment or cancellation. For reporting
	ReturnToStock               string    `bun:"return_to_stock,type:char(1),nullzero"`       // When items is undershipped or cancelled, flag on what to do with the material. For reporting.
	ReasonCode                  int32     `bun:"reason_code,type:int,nullzero"`               // When not returned to stock, explains why. For reporting.
