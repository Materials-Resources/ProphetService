...
type Process struct {
	...
	ProcessDescription      string    `bun:"process_description,type:varchar(255),nullzero"`                // What is the extended description for this process?
	...
	LocationId              float64   `bun:"location_id,type:decimal(19,0),nullzero"`                       // Location for the process
	...
	RawInvMastUid           int32     `bun:"raw_inv_mast_uid,type:int,nullzero"`                            // Unique identifier for the raw item used in this process
	FinishedInvMastUid      int32     `bun:"finished_inv_mast_uid,type:int,nullzero"`                       // Unique identifier for the finished item created through this process
	...
	RawItemRevisionUid      int32     `bun:"raw_item_revision_uid,type:int,nullzero"`                       // Column holds item revision uid of raw item
	FinishedItemRevisionUid int32     `bun:"finished_item_revision_uid,type:int,nullzero"`                  // Column holds item revision uid of finished item
	ProductionOrderFlag     string    `bun:"production_order_flag,type:char(1),nullzero"`                   // Pre-Defined Routing specific of Production Order process (On the Fly created in Assembly Maintenance)
	OneOffFlag              string    `bun:"one_off_flag,type:char(1),nullzero"`                            // Pre-Defined Routing specific of Production Order process (On the Fly created in Assembly Maintenance) that will only be used for the assembly associated with
}
