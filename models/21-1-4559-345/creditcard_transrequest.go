...
type CreditcardTransrequest struct {
	...
	ReqfileLocation           string    `bun:"reqfile_location,type:varchar(255),nullzero"`                 // The path to where the request file was created
	RequesttypeId             int32     `bun:"requesttype_id,type:int"`                                     // ID of credit card transaction type.  The ID is from creditcard_transtype.
	RequestStatus             string    `bun:"request_status,type:varchar(10),nullzero"`                    // Status of the credit card transaction request
