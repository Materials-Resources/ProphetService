...
type TaxJurisdiction struct {
	...
	LineItemLimit               float64   `bun:"line_item_limit,type:decimal(19,4),nullzero"`                   // The maximum amounts to which tax can be charged per line item in a specific jurisdiction.
	InvoiceLimit                float64   `bun:"invoice_limit,type:decimal(19,4),nullzero"`                     // The maximum amounts to which tax can be charged per invoice in a specific jurisdiction.
	FreightInTaxable            string    `bun:"freight_in_taxable,type:char(1),nullzero"`                      // incoming freight is taxable
	FreightOutTaxable           string    `bun:"freight_out_taxable,type:char(1),nullzero"`                     // outgoing freight is taxable
	...
	TaxAmountPerUnit            float64   `bun:"tax_amount_per_unit,type:decimal(19,9),nullzero"`               // A flat tax rate that is applied per unit.
	UnitOfMeasure               string    `bun:"unit_of_measure,type:varchar(8),nullzero"`                      // What is the unit of measure for this row?
	AfterUnits                  float64   `bun:"after_units,type:decimal(19,0),nullzero"`                       // Tax amount that applies after this number of units purchased per order
	AfterMaximumUnits           float64   `bun:"after_maximum_units,type:decimal(19,0),nullzero"`               // Tax amount that applies after this max units purchased per month
	SeeSchedule                 string    `bun:"see_schedule,type:char(1),nullzero"`                            // Indicates whether the tax jurisdiction is setup to use the schedule option.
	TaxOnGrossNetQtyInvoiced    string    `bun:"tax_on_gross_net_qty_invoiced,type:char(1),nullzero"`           // Taxed based on gross/net/units invoiced
	...
	MinLineItemRange            float64   `bun:"min_line_item_range,type:decimal(19,4),nullzero"`               // Min Line Item Range
	MaxLineItemRange            float64   `bun:"max_line_item_range,type:decimal(19,4),nullzero"`               // Max Line Item Range
	...
	ReportCodeUid               int32     `bun:"report_code_uid,type:int,nullzero"`                             // Allows a reporting code to be associated with the jurisdiction
	...
	TaxBeforeAfterRestockFee    string    `bun:"tax_before_after_restock_fee,type:varchar(255),nullzero"`       // This column will indicate in RMAs, if we should calculate tax after or before adding in the Restock Fee.
	OrderLimit                  float64   `bun:"order_limit,type:decimal(19,4),nullzero"`                       // Indicate the order total limit that could be charged tax for the jurisdiction.
	VatFlag                     string    `bun:"vat_flag,type:char(1),nullzero"`                                // For VAT (value added tax) enabled users, determines if this is a VAT jurisdiction.
	...
	JurisdictionTaxTypeCd       int32     `bun:"jurisdiction_tax_type_cd,type:int,nullzero"`                    // Jurisdiction Tax Type
