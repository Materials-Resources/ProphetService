...
type OeLineLastMarginPrice struct {
	...
	PrevSkuPrice             sql.NullFloat64 `bun:"prev_sku_price,type:decimal(19,9),nullzero"`
	PrevQtyOrdered           sql.NullFloat64 `bun:"prev_qty_ordered,type:decimal(19,9),nullzero"`
	PrevProfitPercent        sql.NullFloat64 `bun:"prev_profit_percent,type:decimal(19,9),nullzero"`
	OneTimePrice             string          `bun:"one_time_price,type:char"`
	DateCreated              time.Time       `bun:"date_created,type:datetime,default:(getdate())"`
	CreatedBy                string          `bun:"created_by,type:varchar(255),default:(suser_sname())"`
	DateLastModified         time.Time       `bun:"date_last_modified,type:datetime,default:(getdate())"`
	LastMaintainedBy         string          `bun:"last_maintained_by,type:varchar(255),default:(suser_sname())"`
	CostBasisCdUsed          sql.NullInt32   `bun:"cost_basis_cd_used,type:int,nullzero"`
