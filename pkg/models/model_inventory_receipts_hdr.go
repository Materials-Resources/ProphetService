...
type InventoryReceiptsHdr struct {
	...
	GroupPoHdrUid           sql.NullInt32   `bun:"group_po_hdr_uid,type:int,nullzero"`
	RfnavTransNo            sql.NullInt32   `bun:"rfnav_trans_no,type:int,nullzero"`

	InventoryReceiptsLines []InventoryReceiptsLine `bun:"rel:has-many,join:receipt_number=receipt_number"`
}

type InventoryReceiptsHdrModel struct {
	bun bun.IDB
}

// GetByReceiptNumberWithLines returns a single InventoryReceiptsHdr by the given receipt number
func (m InventoryReceiptsHdrModel) GetByReceiptNumberWithLines(
	ctx context.Context, receiptNumber float64) (*InventoryReceiptsHdr, error) {
	var inventoryReceiptsHdr InventoryReceiptsHdr
	err := m.bun.NewSelect().Model(&inventoryReceiptsHdr).
		Relation("InventoryReceiptsLines").
		Where(
			"receipt_number = ?", receiptNumber).Scan(ctx)
	if err != nil {
		switch {
	...
	return &inventoryReceiptsHdr, nil
}
